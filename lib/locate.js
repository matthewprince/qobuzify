...
// HIGHEST version, not the most recently modified one (an old dir touched by
// AV/indexing would win a pure mtime sort and the patch would land on a dir
// Qobuz never launches). Sort by parsed numeric version fields; mtime only
// breaks exact-version ties.
function versionParts(name) {
  return name.replace(/^app-/, "").split(".").map((n) => parseInt(n, 10) || 0);
}
...
function currentAppDir(root) {
  const dirs = fs.readdirSync(root, { withFileTypes: true })
    ...
    .map((e) => {
      ...
      return { name: e.name, full, ver: versionParts(e.name), mtime: fs.statSync(full).mtimeMs };
    })
    .sort((a, b) => compareVersions(b.ver, a.ver) || b.mtime - a.mtime);
  return dirs[0] || null;
}
