...
**Seeks land on whole seconds; position reads only look smooth.** The engine commits seeks at whole-second granularity, so when seeking to a lyric, round up to the next second unless the line starts in the first 150 ms (a plain round lands before the line about half the time and the previous line highlights). Position reads are smoother than the clock underneath: `Q.player.getPositionMs` wall-clock-extrapolates the store's anchor, millisecond-smooth between refreshes, but the anchor's own refresh cadence is coarse and not reliably characterized (measured claims range from ~250 ms to 1 s). qobuzify-lyrics' smoothing constants assume ~250 ms; the real cadence is still an open measurement.
